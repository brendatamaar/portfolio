...
const JS_KEYWORDS = /\b(const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|new|delete|typeof|instanceof|in|of|import|export|default|from|class|extends|super|this|null|undefined|true|false|try|catch|finally|throw|async|await|yield|static|get|set|void)\b/;
...
const PY_KEYWORDS = /\b(def|return|if|elif|else|for|while|import|from|as|class|pass|break|continue|raise|try|except|finally|with|lambda|yield|None|True|False|and|or|not|in|is|global|nonlocal|del|assert|async|await)\b/;
const GO_KEYWORDS = /\b(func|return|if|else|for|range|switch|case|break|continue|var|const|type|struct|interface|import|package|defer|go|chan|select|map|make|new|nil|true|false|error|string|int|int8|int16|int32|int64|uint|uint8|uint16|uint32|uint64|float32|float64|bool|byte|rune|append|len|cap|close|delete|copy|panic|recover|print|println)\b/;
const RUST_KEYWORDS = /\b(fn|let|mut|return|if|else|for|while|loop|match|use|mod|pub|struct|enum|impl|trait|type|const|static|self|Self|super|crate|where|in|ref|move|async|await|dyn|Box|Option|Some|None|Result|Ok|Err|Vec|String|str|bool|i8|i16|i32|i64|i128|u8|u16|u32|u64|u128|f32|f64|usize|isize|true|false)\b/;
...
const rules: Record<string, HlRule[]> = {
  ...
  python: [
    ...
    { kind: 'builtin', pattern: /\b(print|len|range|enumerate|zip|map|filter|list|dict|set|tuple|int|float|str|bool|type|isinstance|hasattr|getattr|setattr|open|super|property|staticmethod|classmethod)\b/ },
  ...
  css: [
    ...
    { kind: 'builtin', pattern: /#[0-9a-fA-F]{3,8}|\b(\d+)(px|em|rem|%|vh|vw|deg|s|ms)\b/ },
  ...
  bash: [
    { kind: 'comment', pattern: /#.*$/ },
    { kind: 'string', pattern: /"(?:[^"\\]|\\.)*"|'[^']*'/ },
    { kind: 'keyword', pattern: /\b(if|then|else|elif|fi|for|while|do|done|case|esac|in|function|return|exit|export|source|echo|local|declare|readonly|shift|set|unset)\b/ },
    { kind: 'variable', pattern: /\$\{?[\w]+\}?/ },
    { kind: 'builtin', pattern: /\b(cd|ls|mkdir|rm|cp|mv|cat|grep|sed|awk|find|chmod|chown|curl|wget|git|npm|pnpm|node|python|pip)\b/ },
    { kind: 'operator', pattern: /[|&;><]/ },
  ],
  markdown: [
